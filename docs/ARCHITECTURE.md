...
- `srv/internal/domain` — Pure types: Document, Page, TreeNode, TreeIndex, Query, QueryResult, Evidence, HistoryEntry, ReasoningStep.
...
- `srv/internal/api` — HTTP layer: Chi/Huma router, document, query, and history handlers, middleware (including optional CORS).
- `srv/internal/worker` — Bounded goroutine pool for background jobs.
- `srv/internal/config` — Environment variable loading.
- `srv/internal/cli` — CLI application: server run command and OpenAPI spec generation.
...
- `web/src/components/` — UI components: SearchInput, FileUpload, PdfViewer, PreviewDialog, ReferenceListItem, QueryAnswerSection, MirageLogo, Header, Footer, ThemeToggle.
- `web/src/components/ui/` — Shared Radix-based primitive elements (Button, Dialog, Input, Select, etc.).
- `web/src/lib/` — API client (`api.ts`, `api.gen.ts`), domain types (`types.ts`), PDF utilities (`pdfjs.ts`, `pdfHighlighting.ts`, `pdfPageSelection.ts`, `pdfFiles.ts`), evidence processing (`evidence.ts`), preview dialog state (`previewDialogState.ts`).
- `web/src/hooks/` — React Query hooks for data fetching (`documents.ts`) and query history (`history.ts`).
...
- `video/src` — Remotion compositions, root registration (`Root.tsx`), scene modules (`scenes/`), shared components (`components/`), and animation/constant utilities (`lib/`).
...
| GET | `/api/v1/history` | List query history | Returns recent queries with results |
| DELETE | `/api/v1/history` | Clear query history | Returns 204 |

Domain types defined in `srv/internal/domain/`: Document, Page, TreeNode, TreeIndex, Query, QueryResult, Evidence, HistoryEntry, ReasoningStep. OpenAPI spec is auto-generated by Huma at runtime; TypeScript types are generated via `task codegen`.
...
| `HISTORY_MAX_ENTRIES` | `10` | Maximum stored query history entries |
